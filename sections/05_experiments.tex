...
\begin{figure*}[t]
  ...
  \caption{ 
  \textbf{Multimodality Comparison. TO FINISH THE CAPTION AND TO FIX THE NAME}  
  }
...
\begin{wraptable}[8]{r}{0.48\columnwidth}
  ...
  \begin{tabular}{lcc}
  \toprule
  \rowcolor{lightgray} \textbf{Policy} & \textbf{$\mathcal{M}$} & $\mathbf{L_q(\phi)}$ \\
  \midrule
...
% \toprule
% \rowcolor{lightgray}  & \multicolumn{4}{c||}{\textbf{G1}} & \multicolumn{4}{c}{\textbf{G2}} \\
...
% % \caption{Your caption goes here.}
% \toprule
% \rowcolor{lightgray} & \multicolumn{4}{c}{\textbf{G2}} \\
...
% Table~\ref{tab:toy_results} reports results for both goals. Among baselines, the residual policy (\texttt{RES}) achieves the strongest performance, solving \textbf{G1} and retaining two modes in \textbf{G2}, preserving the original multimodality by applying constrained corrections. Diffusion-based methods (\texttt{DPPO}, \texttt{DPPO[10]}) improve task success, with \texttt{DPPO} benefiting from additional denoising steps, though both collapse to fewer modes when the reward deviates from the demonstrations. Steering alone (\texttt{DSRL}) is the least effective, with limited success in \textbf{G1} and complete collapse in \textbf{G2}. Despite some success in retaining multimodality, these trends degrade in the unbalanced setting, where rewards favor a subset of goals and even the stronger baselines collapse to dominant peaks. These results suggest that the further the reward landscape deviates from the implicit distribution of the demonstrations, the harder it becomes to preserve multimodality during fine-tuning. 

Tables~\ref{tab:toy_results_g1} and~\ref{tab:toy_results_g2} report results for \textbf{G1} and \textbf{G2}, respectively. Among baselines, the residual policy (\texttt{RES}) performs best, solving \textbf{G1} and retaining two modes in \textbf{G2}, as tuning the magnitude of the residual corrections to small values helps preserve multimodality. Diffusion-based methods (\texttt{DPPO}, \texttt{DPPO[10]}) improve task success, with \texttt{DPPO} aided by extra denoising steps, but both collapse to fewer modes when rewards diverge from demonstrations. Steering alone (\texttt{DSRL}) is least effective, with limited success in \textbf{G1} and full collapse in \textbf{G2}. %Multimodality retention further degrades in the unbalanced setting, where reward bias toward specific goals causes even strong baselines to collapse to dominant peaks. 
